...
Most of the heavy lifting (snapshot and restore) is done by a single POSIX shell script which must be simple enough to be run by [BusyBox]() inside the [initial ram disk](). The transfer of snapshots to a server and from the server to clients is done by Python scripts with minimal depencencies. The software should therefore work with most distributions.
